...
omw_app_definition_ready() {
	...
	[[ -n "${APP_VERSIONS[$appname]:-}" && -n "${APP_URLS[$appname]:-}" ]] || return 1
	[[ -n "${APP_EXECUTABLE_NAME[$appname]:-}" || -n "${APP_BIN_DIRS[$appname]:-}" ]] ||
		declare -F "_omw_app_install_$appname" >/dev/null
}
...
omw_install_app() {
	...
	local url
	local exec_name="${APP_EXECUTABLE_NAME[$appname]}"
	if ! omw_app_definition_ready "$appname"; then
		omw_log "Unknown app or incomplete app definition: $appname" "ERROR"
		return 1
	fi
	url=$(omw_get_app_url "$appname") || {
		omw_log "Unable to resolve URL for app: $appname" "ERROR"
		return 1
	}
	local install_dir
	...
	if [[ -n "$exec_name" && -z "$bin_dirs" && -L "$symlink_path" && "$force" == "false" ]]; then
...
omw_clean_app_installs() {
	local dry_run="${1:-false}"
	local path
	for path in "$APPS_INSTALL_PATH" "$SCRIPTS_BIN_PATH"; do
