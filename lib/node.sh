...
_omw_node_cache_archive_path() {
	printf '%s/npm-cache-%s.tar.gz' "$PACKAGES_PATH" "$OMW_VERSION"
}
...
_omw_node_restore_cache_archive() {
	local cache_dir archive_path stage_dir
	cache_dir=$(_omw_node_cache_dir)
	archive_path=$(_omw_node_cache_archive_path)
	stage_dir="$BUILDS_PATH/node/.tmp-npm-cache-$$"
	...
	if [[ ! -d "$stage_dir/$(basename "$cache_dir")" ]]; then
		omw_log "npm cache archive does not contain $(basename "$cache_dir")." "ERROR"
		omw_safe_rm_rf "$stage_dir"
		return 1
	fi
	omw_tx_begin || {
	...
	omw_tx_backup_internal "$cache_dir" || {
		omw_tx_rollback
		return 1
	}
...
_omw_node_write_cache_archive() {
	local cache_dir archive_path tmp_archive old_sha new_sha
	cache_dir=$(_omw_node_cache_dir)
	archive_path=$(_omw_node_cache_archive_path)
	...
	if [[ -f "$archive_path" ]]; then
		old_sha=$(omw_file_sha256 "$archive_path") || {
			rm -f "$tmp_archive"
			return 1
		}
		new_sha=$(omw_file_sha256 "$tmp_archive") || {
			rm -f "$tmp_archive"
			return 1
		}
		if [[ "$old_sha" == "$new_sha" ]]; then
			rm -f "$tmp_archive"
			omw_log "npm cache archive is unchanged; keeping existing $archive_path" "INFO"
			return 0
		fi
		rm -f "$archive_path"
	fi
...
_omw_node_cleanup_temp_verify_cache() {
	local mode="$1"
	local old_cache_override="$2"
	local temp_root="$3"

	[[ "$mode" == "temp" && -n "$temp_root" ]] || return 0
	_omw_node_restore_cache_override "$old_cache_override" "$temp_root"
}

_omw_node_verify_npm_cache() {
	local cache_dir="$1"
...
omw_node_verify() {
	...
	local cache_dir alias verify_cache_dir old_cache_override verify_temp_root
	...
	for alias in "${NODE_PACKAGE_LIST[@]}"; do
		if ! _omw_node_verify_global_alias "$alias" "$cache_dir"; then
			_omw_node_cleanup_temp_verify_cache "$mode" "${old_cache_override:-}" "${verify_temp_root:-}"
			return 1
		fi
	done
	...
	for alias in "${NODE_CACHE_PACKAGE_LIST[@]}"; do
		if ! _omw_node_verify_cache_alias "$alias" "$cache_dir"; then
			_omw_node_cleanup_temp_verify_cache "$mode" "${old_cache_override:-}" "${verify_temp_root:-}"
			return 1
		fi
	done
...
omw_node_clean_generated_assets() {
	local dry_run="${1:-false}"
	local path
	for path in "$(_omw_node_cache_archive_path)" "$BUILDS_PATH/node"; do
