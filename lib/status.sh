...
_omw_status_config_install_status() {
	...
	case "$target" in
	tmux)
		[[ -L "$HOME/.tmux.conf" && -L "$HOME/.tmux.conf.local" ]] && printf 'installed' || printf 'available'
		;;
	vim)
		[[ -f "$CONFIG_RELEASE_PATH/vim/vimrc.default" && -f "$HOME/.vimrc" ]] && printf 'installed' || printf 'available'
		;;
	zsh)
		[[ -L "$HOME/.oh-my-zsh" && -f "$HOME/.zshrc" ]] && printf 'installed' || printf 'available'
		;;
	*)
		printf 'available'
		;;
