...
declare -p SOFTWARE_VERSIONS >/dev/null 2>&1 || declare -A SOFTWARE_VERSIONS
declare -p SOFTWARE_URLS >/dev/null 2>&1 || declare -A SOFTWARE_URLS
declare -p SOFTWARE_DEPS >/dev/null 2>&1 || declare -A SOFTWARE_DEPS
declare -p SOFTWARE_CONFIG_CMDS >/dev/null 2>&1 || declare -A SOFTWARE_CONFIG_CMDS
declare -p SOFTWARE_CFLAGS >/dev/null 2>&1 || declare -A SOFTWARE_CFLAGS
declare -p SOFTWARE_LDFLAGS >/dev/null 2>&1 || declare -A SOFTWARE_LDFLAGS
declare -p APP_VERSIONS >/dev/null 2>&1 || declare -A APP_VERSIONS
declare -p APP_URLS >/dev/null 2>&1 || declare -A APP_URLS
declare -p APP_EXECUTABLE_NAME >/dev/null 2>&1 || declare -A APP_EXECUTABLE_NAME
declare -p APP_SOURCE_URLS >/dev/null 2>&1 || declare -A APP_SOURCE_URLS
declare -p APP_BIN_DIRS >/dev/null 2>&1 || declare -A APP_BIN_DIRS
declare -p NODE_PACKAGE_NAMES >/dev/null 2>&1 || declare -A NODE_PACKAGE_NAMES
declare -p NODE_PACKAGE_VERSIONS >/dev/null 2>&1 || declare -A NODE_PACKAGE_VERSIONS
declare -p NODE_PACKAGE_BINS >/dev/null 2>&1 || declare -A NODE_PACKAGE_BINS
declare -p NODE_PACKAGE_NODE_VERSIONS >/dev/null 2>&1 || declare -A NODE_PACKAGE_NODE_VERSIONS
declare -p NODE_CACHE_PACKAGE_NAMES >/dev/null 2>&1 || declare -A NODE_CACHE_PACKAGE_NAMES
declare -p NODE_CACHE_PACKAGE_VERSIONS >/dev/null 2>&1 || declare -A NODE_CACHE_PACKAGE_VERSIONS
...
#   5. build_cmd  Configure command template, or "-" for a dedicated _omw_build_<name> hook.
...
#   4. executable  Executable name to expose, or "-" for a dedicated installer hook.
...
#
# Ordinary apps expose executable or bin_dirs. Apps without either must provide
# a dedicated _omw_app_install_<name> hook in lib/app.sh.
#
# node_package fields:
...
app "exa" \
	"0.10.1" \
	"https://github.com/ogham/exa/releases/download/v${VERSION}/exa-linux-x86_64-musl-v${VERSION}.zip" \
	"exa" \
	"-" \
	"-"
...
app "yazi" \
	"26.5.6" \
	"https://github.com/sxyazi/yazi/releases/download/v${VERSION}/yazi-x86_64-unknown-linux-musl.zip" \
	"yazi" \
	"-" \
	"-"
...
app "sd" \
	"1.1.0" \
	"https://github.com/chmln/sd/releases/download/v${VERSION}/sd-v${VERSION}-x86_64-unknown-linux-musl.tar.gz" \
	"sd" \
	"-" \
	"-"
...
app "autojump" \
	"22.5.3" \
	"https://github.com/wting/autojump/archive/refs/tags/release-v${VERSION}.zip" \
	"-" \
	"-" \
	"-"
...
app "btop" \
	"1.4.7" \
	"https://github.com/aristocratos/btop/releases/download/v${VERSION}/btop-x86_64-unknown-linux-musl.tar.gz" \
	"btop" \
	"-" \
	"-"
...
app "hack-nerd-font" \
	"3.4.0" \
	"https://github.com/ryanoasis/nerd-fonts/releases/download/v${VERSION}/Hack.tar.xz" \
	"-" \
	"-" \
	"-"
...
app "verible" \
	"0.0-4053-g89d4d98a" \
	"https://github.com/chipsalliance/verible/releases/download/v${VERSION}/verible-v${VERSION}-linux-static-x86_64.tar.gz" \
	"verible-verilog-ls" \
	"-" \
	"bin"
...
# Cache-only packages are included in packages/npm-cache-$OMW_VERSION.tar.gz but are not
